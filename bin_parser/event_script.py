...
Frame format (empirically verified across multiple files):

    <op_u16_BE>  <length_u8>  <payload[length]>

So each command is 3 + length bytes. Opcodes overwhelmingly have the
form `0xXX00` -- the low byte is a 0x00 padding -- so we treat opcode
as a 1-byte id (the high byte of the u16 frame header).
...
def walk_script(b, start, file_end):
    ...
    Each dict has: `offset`, `op` (u8), `op_u16` (raw u16 BE), `name`,
    `length`, `payload_hex`, plus any fields the payload decoder adds.
    The walker stops with a `_stop` record when:
      * the length field would overrun `file_end`, or
      * the high byte of the opcode word is non-zero (typically means
        we've fallen out of the script into padding/data).
    """
    o = start
    while o + 3 <= file_end:
        op_word = struct.unpack_from(">H", b, o)[0]
        op = op_word >> 8
        op_lo = op_word & 0xff
        length = b[o + 2]
        end = o + 3 + length
        # Bail-out heuristics: real opcodes have low byte 0. A non-zero
        # low byte typically signals the walker has fallen out of the
        # script into padding/data.
        if op_lo != 0:
            yield {
                "_stop": "non_canonical_opcode",
                "offset": o,
                "op_u16": op_word,
            }
            return
