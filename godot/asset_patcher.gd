...
func _patch_next_file():
	if current_patch_index >= files_to_patch.size():
		patch_complete.emit()
		return
		
	var file_type = files_to_patch[current_patch_index]
	...
	var rpc_payload = JSON.stringify({"type": file_type})
	var result = await server_connection.get("_client").rpc_async(server_connection.get("_session"), "get_data_version", rpc_payload)
	...
	var server_version = dict["version"]
	var download_url = dict["download_url"]
	
	var local_version = _get_local_version(file_type)
...
func _download_file(file_type: String, url: String, new_version: String):
...
func _on_request_completed(result, response_code, headers, body):
	var file_type = _http_request.get_meta("file_type")
	var new_version = _http_request.get_meta("new_version")
...
func _save_versions_file(versions: Dictionary):
	var path = "user://data/versions.json"
...
func _save_to_cache(file_type: String, json_string: String):
	var path = "user://data/%s.json" % file_type
	var file = FileAccess.open(path, FileAccess.WRITE)
	if file:
		file.store_string(json_string)
		file.close()

func _load_from_cache(file_type: String):
	var path = "user://data/%s.json" % file_type
