...
@onready var next_button: TextureButton = $ContentRoot/VBoxContainer/MissionChallengesPanel/NextButton
...
func _populate_challenge_rows(mission: Dictionary) -> void:
	...
	_set_section_visible(initial_header, initial_rows_host, sections["initial"], objectives, show_stars, 0)
	var achievement_offset: int = sections["initial"].size()
	_set_section_visible(achievement_header, achievement_rows_host, sections["achievement"], objectives, show_stars, achievement_offset)
...
func _set_section_visible(
	...
	for index in range(challenges.size()):
		...
		row.configure(task_text, lapis_amount, completed, show_stars)
