...
func _populate_skills(final_stats_profile: Dictionary) -> void:
	...
	for sk in magic_list:
		...
		if not magic_data.is_empty():
			...
			panel.setup_from_skill_data(magic_data, str(sk.get("source", "Trait")), sk.get("awaken_level", 0), false)
			unit_detail_magic_grid.add_child(panel)

	var ability_list: Array = all_skills.get("ability", [])
	for sk in ability_list:
		var ability_data: Dictionary = GameDatabase.get_ability(str(sk.get("id", "")))
		if not ability_data.is_empty():
			var panel: Control = MagicScene.instantiate()
			panel.setup_from_skill_data(ability_data, str(sk.get("source", "Trait")), sk.get("awaken_level", 0), false)
			unit_detail_special_grid.add_child(panel)

	var passive_list: Array = all_skills.get("passive", [])
	for sk in passive_list:
		var passive_data: Dictionary = GameDatabase.get_passive(str(sk.get("id", "")))
		if not passive_data.is_empty():
			var panel: Control = MagicScene.instantiate()
			panel.setup_from_skill_data(passive_data, str(sk.get("source", "Trait")), sk.get("awaken_level", 0), false)
			unit_detail_special_grid.add_child(panel)
