...
TOWN_DATA_ROOT = REPO_ROOT / "godot" / "assets" / "town_data"
# TOWN_DATA_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
#                               "assets", "town_data")


# --- Scripted entity identity slot -------------------------------------------
# Every NPC-bearing scripted_entity carries one u32 BE "identity" value that is
# either a texture id (the `npc<id>.png` spritesheet) or an npc instance id --
# the per-map handle that `npc.npcId`, `quest.npcId` and the dialogue table's
# `<name_npc=N>` tags all reference.
#
# The slot sits at payload offset 4 or 6 (record offset 29 or 31): some records
# carry an extra 2-byte flag word between the `0101` header word and the slot.
# Nothing in the class tag distinguishes the two shapes -- `00BA` records appear
# with both -- so probe both offsets and take the one whose u32 lands in a
# plausible id band. Reading a +6 record at +4 yields `0101 xxxx` (~1.7e7) and
# reading a +4 record at +6 yields `xxxx ????` in the 1.2e9 range, so exactly
# one offset validates.
#
# The old decoder hardcoded +6, which silently produced garbage ids for every
# +4 town (111020100, 111010700, 112020100 in the current corpus): their NPCs
# got no sprite at all and their instance ids matched nothing.
NPC_TEXTURE_ID_MIN = 100_000_000
NPC_TEXTURE_ID_MAX = 900_300_000
NPC_INSTANCE_ID_MIN = 900_000
NPC_INSTANCE_ID_MAX = 9_999_999
IDENTITY_SLOT_OFFSETS = (4, 6)


def _find_identity_slot(payload):
    """Locate the identity u32 in a scripted_entity payload.

    Returns (offset, value), or (None, None) when neither candidate offset
    holds a plausible id -- which is the correct answer for the scripted
    records that are pure triggers rather than NPCs.
    """
    for off in IDENTITY_SLOT_OFFSETS:
        if off + 4 > len(payload):
            continue
        value = struct.unpack(">I", payload[off:off + 4])[0]
        if (NPC_TEXTURE_ID_MIN <= value <= NPC_TEXTURE_ID_MAX
                or NPC_INSTANCE_ID_MIN <= value <= NPC_INSTANCE_ID_MAX):
            return off, value
    return None, None


def _split_scripted_payload(payload, ident_off=6):
    """Decompose a scripted_entity payload into bundled hex regions.

    The payload's fixed prefix splits naturally around the already-decoded
    identity slot at `payload[ident_off:ident_off + 4]` (sprite_id XOR
    npc_instance_id, handled by the caller -- see `_find_identity_slot`).
    Everything before the slot is one contiguous header region; everything
    from just after it up to the first tail-block marker is one contiguous
    ...
    Internal byte layout (relative to payload start):
      header_hex  [0:I]    bytes 0-1  class tag (00BA / 00C8)
                            ...
                            bytes 4-5  optional flag word (0101 / 0201),
                                       present only when I == 6
      [I:I+4]              sprite_id  XOR  npc_instance_id  (caller)
      body_hex    [I+4:T]  first 4 bytes usually zeros
                            then       0104 / 0004
                            then       usually 0x14
                            then       flag bytes, a 4-byte flag block,
                                       and scratch / per-instance event id
    ...
    if n >= 1:
        out["header_hex"] = payload[0:min(ident_off, n)].hex().upper() or None
    if n > ident_end:
        body_end = min(tail_start, n)
        body_bytes = payload[ident_end:body_end]
    ...
    for layer in blueprint.get("layers", []):
        for ent in layer.get("objects", {}).get("dynamic_entities", []):
            ...
            try:
                ...
                continue
            # The identity u32 (see `_find_identity_slot`) is either a
            # texture id, matching an `npc<id>.png` spritesheet, or an
            # `npc_instance_id` -- the per-map NPC handle that `npc.npcId`,
            # `quest.npcId` and the dialogue table's `<name_npc=N>` tag all
            # reference. Verified by cross-checking against datamined
            # per-town reference files (e.g. quest-giver 0x0010D0EC ==
            # 1102060 == the Hill Gigas mission giver in town 111020200).
            ident_off, identity = _find_identity_slot(payload)
            if identity is None:
                ident_off = 6  # keep the historic split for trigger records
            else:
                ent["identity_offset"] = ident_off
                if NPC_TEXTURE_ID_MIN <= identity <= NPC_TEXTURE_ID_MAX:
                    ent["sprite_id"] = identity
                else:
                    ent["npc_instance_id"] = identity
                    ent["npc_instance_id_hex"] = payload[
                        ident_off:ident_off + 4].hex().upper()
            # Split the rest of the payload into atomic named fields
            # so unknown bytes can be pattern-hunted by the frontend.
            ent.update(_split_scripted_payload(payload, ident_off))
...
# --- Treasure chests ---------------------------------------------------------
# A chest record (type 0x02) carries two u32 references, both readable
# straight from the bin:
#
#   `01 <u32>`                     the chest's open-state switch. Every
#                                  chest-family switch id lives in the
#                                  40,000,000 - 53,999,999 band, and
#                                  `switch.switchType` then says which
#                                  flavour it is: 400/401 chest, 410/411
#                                  locked chest, 500/501 medal, 510 crest,
#                                  520 strongbox.
#   `4D 00 05 <u32> 01`            primary key into the `field_treasure`
#                                  master, which holds the reward as
#                                  "type:id:count:rate".
#
# The previous decoder matched three literal byte prefixes (`01 02 62`,
# `01 03 19 9C`, `01 03 0A 59`) plus a `4D 00 05 00 10` tail, and stored a
# signed offset byte and the low u16 of the id. Those literals are only the
# high bytes of the two ids, so they matched just the towns whose ids happen
# to start with them -- 53 of the 2667 field_treasure rows have a treasureId
# beginning 0x0010. Matching on value range instead covers every town and
# yields both ids whole.
CHEST_SWITCH_MIN = 40_000_000
CHEST_SWITCH_MAX = 53_999_999
# `[tag] 00 05 <u32> 01` is a generic typed reference block; the tag names the
# field. Town maps tag a treasure id 0x4D, dungeon maps tag it 0x48 -- the same
# payload otherwise, confirmed by the chest switch that accompanies it. 0x0A is
# the same shape carrying a plain switch id (see _extract_inline_switch_refs).
TREASURE_ID_TAGS = (0x4D, 0x48)
REFERENCE_BLOCK_MARKER = b"\x00\x05"


def _chest_switch_refs(buf):
    """Every `01 <u32 BE>` in `buf` whose u32 is a chest-family switch id.

    Returns a list of (offset, switch_id) in ascending offset order. A real
    chest record carries exactly one; the list form lets the collapsed-record
    recovery below pick the reference nearest a given tail marker.
    """
    out = []
    i = buf.find(b"\x01")
    while i >= 0 and i + 5 <= len(buf):
        value = struct.unpack(">I", buf[i + 1:i + 5])[0]
        if CHEST_SWITCH_MIN <= value <= CHEST_SWITCH_MAX:
            out.append((i, value))
        i = buf.find(b"\x01", i + 1)
    return out


def _find_treasure_id(buf, start=0):
    """First `<tag> 00 05 <u32 BE> 01` treasure block at or after `start`,
    where tag is one of TREASURE_ID_TAGS.

    Returns (offset_of_tag, treasure_id), or (-1, None) when there is none.
    The offset points at the tag byte so callers can resume past the block.
    The trailing 0x01 is the block terminator shared with the other `00 05`
    reference blocks; requiring it keeps the scan off unrelated bytes.
    """
    i = buf.find(REFERENCE_BLOCK_MARKER, max(start + 1, 1))
    while i >= 0:
        if (i + 7 <= len(buf) and buf[i - 1] in TREASURE_ID_TAGS
                and buf[i + 6] == 0x01):
            return i - 1, struct.unpack(">I", buf[i + 2:i + 6])[0]
        i = buf.find(REFERENCE_BLOCK_MARKER, i + 1)
    return -1, None


# The previous decoder also carried an `event_flag_ids` field: a free scan for
# any byte-aligned `01 XX YY ZZ` u32 in the 0x01100000-0x01FFFFFF range, on the
# theory that those were quest/event flags. They are not a separate concept --
# sub-quest switch ids happen to live in 20,000,000-25,000,000, which is
# 0x01312D00-0x017D7840, so a real switch reference does start with 0x01. But
# scanning for the byte pattern rather than the block structure swept up far
# more noise than signal: across the current corpus 834 of the 950 extracted
# values matched no `switch` row at all (the fixed chest-record prefix
# `01 27 10 01 96 00 00 00` alone minted two phantom ids on every chest).
# The structural `0A 00 05 <u32> 01` extraction below resolved 53 of 55 against
# the switch table, so that is the only source kept.
...
def _annotate_inline_switches(blueprint):
    ...
    Every reference is anchored on a recognised byte-block shape; nothing is
    inferred from a bare value range (see the note above
    `_extract_inline_switch_refs`).
    """
    for layer in blueprint.get("layers", []):
        for ent in layer.get("objects", {}).get("dynamic_entities", []):
...
def _decode_interactive_objects(blueprint):
    ...
    `[tag][08 00 04][u32]` marker block in the payload and surface the
    dialogue-line ids among them.
    """
    for layer in blueprint.get("layers", []):
        for ent in layer.get("objects", {}).get("dynamic_entities", []):
...
def _build_quest_chains(blueprint):
    """Group records that reference switches from the same quest.

    Sub-quest switch ids are allocated in per-quest runs (the "Secrets
    Unearthed" chain is 20004011 / 20004012 / 20004013), so records whose
    structural switch references share `switch_id // 100` belong to the same
    quest. The resulting `quest_chains` array is attached at the blueprint
    root; each entry lists the participating records with their layer,
    record_id, kind, and the switch ids they reference.

    Only switches that came out of a recognised reference block are used --
    this used to group on the `event_flag_ids` byte-scan, which produced
    chains out of the fixed bytes every chest record happens to contain.

    NOTE: this catches **object-interaction quests only** (where map.bin
    physically carries the switch ids). Talk-to-N-NPCs quests are tracked
    outside the bin via dialogue_line_id matching and will produce no chain
    here, which is the truthful reflection of the bin's contents.
    """
    chains = {}
    for layer in blueprint.get("layers", []):
        lid = layer.get("layer_id")
        for ent in layer.get("objects", {}).get("dynamic_entities", []):
            switch_ids = ent.get("switch_ids")
            if not switch_ids:
                continue
            for ns in {sid // 100 for sid in switch_ids if isinstance(sid, int)}:
                ...
                chain["members"].append({
                    ...
                    "switch_ids": [s for s in switch_ids if s // 100 == ns],
                })

    # Only keep namespaces with at least 2 members (a single record with a
    # stray switch reference isn't a quest chain).
    quest_chains = [c for c in chains.values() if len(c["members"]) >= 2]
    quest_chains.sort(key=lambda c: c["switch_prefix"])
    if quest_chains:
...
def parse_ffbe_map(file_path, output_path=None, pre_parsed=None):
    ...
    with open(file_path, "rb") as f:
        ...
        while f.tell() < file_end_offset:
            ...
            # 0x0E is the gathering / collection point used by dungeon maps:
            # a 48-byte record whose `07 50 00 04 <u32>` block names the drop
            # pool. It was missing from this set, so the walker never saw the
            # headers and folded every collection point into whichever record
            # preceded it (Earth Shrine's three spots were absorbed by a chest
            # and by the quest trigger).
            DE_KNOWN_TYPES = {
            ...
            # The "minimal" record is the 25-byte common header plus a
            # zero trailer. That trailer is 14 bytes, not 16: several towns
            # pack minimal records 39 bytes apart (111020100 layer 56 chains
            # six of them: rids 35->39, 38->36, 40->41, 42->43, 44->48,
            # 37->7). A floor of 41 stepped straight over every one of those
            # successors, and the walker then absorbed them into the
            # preceding record.
            DE_MIN_LEN_BY_TYPE = {
                0x00: 36, 0x01: 54, 0x02: 70,
                0x04: 39, 0x05: 39, 0x06: 39, 0x07: 39,
                0x08: 39, 0x10: 39, 0x11: 39, 0x14: 46,
                0x16: 39, 0x18: 39, 0x0F: 59, 0x12: 117,
                0x1B: 51, 0x0E: 48,
            }
            ...
            def _de_header_at(payload, off, seen_now, rid_cap):
                ...
                if not (1 <= w <= 2048 and 1 <= h <= 2048):
                    return False
                # Real footprints are almost always multiples of 29
                # (half-tile), and ghost headers usually aren't -- but a few
                # genuine records use other sizes (80x80 NPCs). Rather than
                # reject those outright, fall back to a positive signature:
                # a scripted entity's class word followed by a decodable
                # identity slot is not something ghost bytes produce.
                if w % 29 != 0 or h % 29 != 0:
                    return _has_scripted_signature(payload, off)
                return True

            # Class word that opens every scripted_entity payload, followed
            # by the constant 0101 word (see _split_scripted_payload).
            _SCRIPTED_CLASS_TAGS = (b"\x00\xBA", b"\x00\xC8")

            def _has_scripted_signature(payload, off):
                """True when the record at `off` opens with a scripted
                entity's `<class tag> 0101` word and carries a decodable
                identity slot (texture id or npc instance id)."""
                body = payload[off + 25:off + 25 + 16]
                if len(body) < 10:
                    return False
                if body[0:2] not in _SCRIPTED_CLASS_TAGS or body[2:4] != b"\x01\x01":
                    return False
                return _find_identity_slot(body)[1] is not None
            ...
            try:
                ...
                if de_count == 0:
                    ...
                    if boundaries is None:
                        ...
                        for k, (s, e, _walk_rid, _walk_rt) in enumerate(boundaries):
                            ...
                            # A type-0x02 record is a chest iff it carries BOTH
                            # a treasure-id tail block and a chest-family
                            # switch reference. Either test alone produces
                            # false positives: a run of zero bytes can read as
                            # `01 03 00 00 00` (= 50331648, inside the switch
                            # band but not a real switch id), and the `00 05`
                            # block family is shared with other references.
                            chest_switches = (
                                _chest_switch_refs(rec_bytes)
                                if rec_type == 0x02 else []
                            )
                            chest_tail_off, chest_tail_id = (
                                _find_treasure_id(rec_bytes)
                                if chest_switches else (-1, None)
                            )
                            ...
                            if rec_type in MINIMAL_DE_TYPES and rec_len == 41:
                                ...
                                # Gathering / collection point: walk over it and
                                # it rolls a drop pool. `07 50 00 04 <u32>` names
                                # the pool (Earth Shrine uses 10000/10001/10002);
                                # the pool -> item-and-rate table is not in any
                                # datamine dump we have, so only the id is
                                # exposed here.
                                entity["kind"] = "collection_point"
                                m_pool = rec_bytes.find(b"\x50\x00\x04")
                                if m_pool >= 0 and m_pool + 7 <= rec_len:
                                    entity["collection_pool_id"] = struct.unpack(
                                        ">I", rec_bytes[m_pool + 3:m_pool + 7])[0]
                                entity["payload_hex"] = rec_bytes[25:].hex().upper()
                            elif rec_type == 0x01:
                                entity["kind"] = "scripted_entity"
                                entity["payload_hex"] = rec_bytes[25:].hex().upper()
                            elif rec_type == 0x02 and chest_tail_id is not None:
                                # Treasure chest: a type=0x02 record that
                                # references a chest-family switch. Both ids
                                # the frontend needs are in the record:
                                #   open_switch_id -- `switch` row; its
                                #     switchType classifies the chest and the
                                #     id doubles as the "already opened" flag.
                                #   treasure_id    -- `field_treasure` row,
                                #     which carries the reward.
                                # See the module-level notes above the
                                # `_chest_switch_refs` helper for the layout.
                                entity["kind"] = "chest"
                                entity["open_switch_id"] = chest_switches[0][1]
                                entity["treasure_id"] = chest_tail_id
                                m_ent = chest_tail_off
                                entity["payload_hex"] = rec_bytes[25:].hex().upper()
                                ...
                                while True:
                                    mt, sub_ent = _find_treasure_id(rec_bytes, scan_from)
                                    if mt < 0:
                                        break
                                    sub_entity = {
                                        "record_id": rec_id,
                                    ...
                                    # Look backward up to 100B for this
                                    # chest's own switch reference and the
                                    # source_x/y of the synthetic header that
                                    # precedes the tail. The nearest switch
                                    # reference before the tail is this
                                    # chest's, so take the last one found.
                                    back_window = rec_bytes[max(0, mt - 100):mt]
                                    back_switches = _chest_switch_refs(back_window)
                                    if back_switches:
                                        sub_entity["open_switch_id"] = back_switches[-1][1]
                ...
                # tail markers (`4D 00 05 [u32] 01`) whose absolute
                ...
                _scan_from = 0
                while True:
                    mt, sub_ent = _find_treasure_id(de_payload, _scan_from)
                    if mt < 0:
                        break
                    ...
                    if any(s <= abs_tail <= e for s, e in covered_spans):
                        _scan_from = mt + 8
                        continue
                    # Validate: a chest-family switch reference must sit
                    # within 120B before the tail. The nearest one is this
                    # chest's own open switch.
                    back_window = de_payload[max(0, mt - 120):mt]
                    back_switches = _chest_switch_refs(back_window)
                    if not back_switches:
                        _scan_from = mt + 8
                        continue
                    sub_entity = {
                        "record_id": -1,
                        ...
                        "kind": "chest",
                        "treasure_id": sub_ent,
                        "open_switch_id": back_switches[-1][1],
                        "length_bytes": 0,
    ...
    # parsed marker_blocks + dialogue_line_ids.
...
if __name__ == "__main__":
    town_id = sys.argv[1] if len(sys.argv) > 1 else "111020100"
    parse_town(town_id)